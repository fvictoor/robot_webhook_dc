...
def notificar_slack(link_webhook, texto):
    ...
    if not link_webhook:
        raise ValueError("A variável 'SLACK_WEBHOOK_URL' não foi definida no seu arquivo .env.")
...
def main():
    ...
    if not slack_webhook_url:
        print("[ERRO] A variável SLACK_WEBHOOK_URL não foi encontrada. Verifique seu arquivo .env.")
        sys.exit(1)
    ...
    notificar_slack(slack_webhook_url, relatorio_final)
